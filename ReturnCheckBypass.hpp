...
public:
	template<typename T>
	static T unprotectfunction(BYTE* funcaddr)
	{
		...
		if (disable_retcheck((T)new_func, func_size))
		{
			cache.emplace((T)funcaddr, (T)new_func);
			fix_calls((T)new_func, (T)funcaddr, func_size);
			return (T)(new_func);
		}
	...
	static void fix_calls(DWORD new_func, DWORD orig_func, DWORD func_len)
	{
		...
		while (disasm_ptr - new_func < func_len)
		{
			...
			if (disasm.opcode == 0xE8) //CALL
			{
				...
				{
					DWORD new_called_func = unprotectfunction<DWORD>((BYTE*)orig_called_func);
